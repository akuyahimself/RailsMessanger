...
class MessagesController < ApplicationController
  ...
  def like
    ...
    @message.broadcast_replace_to [current_user, room], target: "message_like_#{@message.id}",
                                                        partial: 'messages/heart', locals: { user: current_user, message: @message.decorate }
    @message.broadcast_replace_to room, target: "message_likes_count_#{@message.id}", partial: 'messages/likes_count',
